...
async def camera_worker(
    ...
    try:
        ...
        # 프레임 유지율 최대화: 버퍼 크기 증가 (CUDA 환경 최적화: 3 -> 5, 프레임 드롭 방지)
        camera.set(cv2.CAP_PROP_BUFFERSIZE, 5)  # 버퍼 증가 (CUDA 환경 프레임 유지율 향상)
    ...
    # 메모리 누수 방지: 주기적 캐시 정리 (CUDA 환경 최적화)
    last_cache_cleanup = time.time()
    CACHE_CLEANUP_INTERVAL = 30.0  # 60 -> 30초 (CUDA 환경 메모리 관리 개선)
    ...
    while True:
        try:
            ...
            # AI 처리 (비동기, CUDA와 CPU 최대 활용)
            try:
                # 프레임 인코딩과 AI 처리를 병렬로 실행 (CPU와 CUDA 동시 활용)
                ...
                # AI 처리를 별도 스레드에서 실행 (CUDA/GPU 활용, CPU와 병렬)
                # frame_processing_executor는 CUDA와 CPU를 최대한 활용하도록 최적화됨
                processed_frame_bytes, result_data = await loop.run_in_executor(
                    frame_processing_executor,  # 전용 프레임 처리 Executor 사용 (CUDA 최대 활용)
                ...
                # 🔗 통합 결과 캐시와 동기화 (중요)
                # ================================
                # frame_processor는 모든 AI 결과를 state.model_results_cache에 저장합니다.
                # 간혹 process_single_frame의 반환값이 빈 리스트인 경우가 있어,
                # WebSocket으로는 항상 빈 결과만 보내지는 문제가 있었습니다.
                #
                # 이를 방지하기 위해, 여기서 최신 캐시 값을 다시 읽어와
                # violations / recognized_faces 를 덮어씌웁니다.
                try:
                    from state import get_latest_cache, CACHE_TTL

                    # 캐시 유효 시간을 CACHE_TTL(3.0초)로 설정 (감지 결과 안정성 확보)
                    cached = get_latest_cache(cam_id, max_age=CACHE_TTL)
                except Exception:
                    cached = None

                if cached:
                    cached_faces = cached.get("recognized_faces", [])
                    cached_violations = cached.get("violations", [])

                    # 캐시에 의미 있는 데이터가 있으면 해당 값을 사용
                    if cached_faces or cached_violations:
                        # 기존 result_data를 보존하면서 감지 결과만 교체
                        result_data["recognized_faces"] = cached_faces
                        result_data["violations"] = cached_violations
                        result_data["violation_count"] = len(cached_violations)
                ...
                with buffer_lock:
                    ...
                    # FPS 계산 및 적응형 프레임 간격 조정 (CUDA/CPU 최대 활용)
                    frame_count += 1
                ...
                if result.get("violations") and len(result.get("violations", [])) > 0:
                    ...
                    for violation in result["violations"]:
                        ...
                        # Unknown 사용자는 이미지 저장 건너뜀
                        if not worker_name or worker_name.lower() in ['unknown', '알수없음', '알 수 없음', '미확인']:
                            logging.debug(f"[CAM-{cam_id}] Unknown 사용자 이미지 저장 건너뜀: worker={worker_name}")
                            continue
                        
                        safe_worker_name = "".join(c for c in worker_name if c.isalnum() or c in ('-', '_'))[:20]
                        ...
                        try:
                            ...
                            if person_box and len(person_box) == 4:
                                ...
                                # 작업자 이름 추가
                                if worker_name and worker_name != "Unknown" and worker_name != "알 수 없음":
                                    text_parts.append(worker_name)
                ...
                # WebSocket으로 브로드캐스트 (main.py의 통합 루프 사용 권장)
...
async def save_violations_to_db(
    ...
    with violation_batch_lock:
        if current_time - state.global_last_saved_time < GLOBAL_SAVE_INTERVAL:
            ...
            logging.debug(f"[MongoDB] 전역 쿨다운 중: {remaining:.1f}초 남음")
            return
    ...
    if not worker_name or worker_name.lower() in ['unknown', '알수없음', '알 수 없음', '미확인']:
        logging.debug(f"[MongoDB] Unknown 사용자 저장 건너뜀: worker={worker_name}")
        return
    ...
    logging.info(f"[MongoDB] 위반 저장: {worker_name} - {violation.get('violations', [])} (CAM-{cam_id})")
...
def get_camera_thumbnail(cam_id: int) -> Optional[bytes]:
    ...
    with buffer_lock:
        ...
        ret, jpeg = cv2.imencode('.jpg', thumbnail, [cv2.IMWRITE_JPEG_QUALITY, 70])
        if ret:
            return jpeg.tobytes()
        return None
