...
# onnxruntime DLL 경로 설정 (다른 모듈보다 먼저 import해야 함)
# CUDA/TensorRT DLL이 제대로 로드되도록 PATH 설정
_conda_env = os.path.dirname(os.path.dirname(os.sys.executable))

# NVIDIA CUDA 라이브러리 경로 (buffalo_l GPU 지원용)
_nvidia_base = os.path.join(_conda_env, 'Lib', 'site-packages', 'nvidia')
_nvidia_dll_paths = [
    os.path.join(_nvidia_base, 'cublas', 'bin'),
    os.path.join(_nvidia_base, 'cudnn', 'bin'),
    os.path.join(_nvidia_base, 'cufft', 'bin'),
    os.path.join(_nvidia_base, 'cusparse', 'bin'),
    os.path.join(_nvidia_base, 'cusolver', 'bin'),
    os.path.join(_nvidia_base, 'curand', 'bin'),
    os.path.join(_nvidia_base, 'cuda_runtime', 'bin'),
    os.path.join(_nvidia_base, 'cuda_nvrtc', 'bin'),
    os.path.join(_nvidia_base, 'nvjitlink', 'bin'),
]

_onnx_dll_paths = [
    os.path.join(_conda_env, 'Library', 'bin'),
    os.path.join(_conda_env, 'Lib', 'site-packages', 'onnxruntime', 'capi'),
] + _nvidia_dll_paths

for _path in _onnx_dll_paths:
    if os.path.exists(_path) and _path not in os.environ.get('PATH', ''):
        os.environ['PATH'] = _path + os.pathsep + os.environ.get('PATH', '')
        # Windows 10 이상에서 DLL 디렉토리 추가
        try:
            os.add_dll_directory(_path)
        except (AttributeError, OSError):
            pass  # Python 3.7 이하 또는 경로 오류

# onnxruntime과 faiss를 가장 먼저 import (DLL 충돌 방지)
try:
    import onnxruntime
    # ONNX Runtime 경고 숨기기 (Fallback mode 경고 등)
    onnxruntime.set_default_logger_severity(3)  # 3 = ERROR only
    
    # InsightFace 경고 숨기기
    import warnings
    warnings.filterwarnings('ignore', message='.*insightface.*')
    warnings.filterwarnings('ignore', message='.*utils.*')
    
    logging.info(f"✅ onnxruntime {onnxruntime.__version__} 로드 성공 (Providers: {onnxruntime.get_available_providers()})")
except Exception as e:
    logging.warning(f"⚠️ onnxruntime import 실패: {e}")

try:
    import faiss
    logging.info(f"✅ FAISS 로드 성공")
except Exception as e:
    logging.warning(f"⚠️ FAISS import 실패: {e}")

# MongoDB 저장 활성화 (기본값: True)
ENABLE_MONGODB_SAVE = os.getenv('ENABLE_MONGODB_SAVE', 'true').lower() == 'true'
if not ENABLE_MONGODB_SAVE:
    ...
    logging.info("[MongoDB] 위반 사항 저장이 활성화되어 있습니다 (ENABLE_MONGODB_SAVE=True)")
...
# torch matmul precision (CUDA 최적화를 위해 조정 가능)
try:
    # CUDA 최적화: medium precision으로 설정 (속도와 정확도 균형)
    # high: 더 정확하지만 느림, medium: 균형, highest: 가장 정확하지만 매우 느림
    precision = os.getenv("TORCH_MATMUL_PRECISION", "medium")
    torch.set_float32_matmul_precision(precision)
    if torch.cuda.is_available():
        logging.info(f"✅ CUDA 최적화: matmul precision={precision}")
...
if torch.cuda.is_available():
    try:
        ...
        if gpu_count >= 2:
            ...
            # 멀티 GPU: RTX 2080 Ti 2대 최대 활용 (11GB x 2 = 22GB 총 VRAM)
            # GPU 사용률 극대화를 위해 워커 수 대폭 증가 (30 FPS 목표, GPU 사용률 85-95% 목표)
            if avg_memory_gb >= 10:
                # RTX 2080 Ti 2대: GPU 사용률 극대화 (30% → 85-95%, 30 FPS 목표)
                # GPU 0: YOLO Violation/Pose (24개 워커 - GPU 활용률 극대화)
                # GPU 1: YOLO Face/InsightFace (28개 워커 - GPU 활용률 극대화)
                _DEFAULT_FACE_WORKERS = 28  # 얼굴 인식 워커 (GPU 1 활용률 극대화, 18→28)
                _DEFAULT_YOLO_WORKERS = 24  # YOLO 워커 (GPU 0 활용률 극대화, 16→24)
                _DEFAULT_DANGER_WORKERS = 16  # 위험 행동 감지 워커 (10→16)
                _DEFAULT_FRAME_WORKERS = 32  # 프레임 처리 워커 (CPU + GPU 병렬 처리, 24→32)
            else:
                _DEFAULT_FACE_WORKERS = 10
                _DEFAULT_YOLO_WORKERS = 8
                _DEFAULT_DANGER_WORKERS = 6
                _DEFAULT_FRAME_WORKERS = 18
            ...
            logging.info(f"⭐ GPU 사용률 극대화 모드 활성화 (목표: 85-95%, 30 FPS 목표)")
...
def update_kpi_from_all_cameras() -> None:
    ...
    with results_lock, frame_lock:
        ...
        for v in all_violations:
            ...
            # PPE 위반이 있는지 확인
            has_ppe_violation = any("안전모" in str(vt) or "마스크" in str(vt) or "안전조끼" in str(vt) for vt in violations)
...
async def save_violation_to_mongodb(worker: str, violations: List[str], area: str, cam_id: int) -> None:
    ...
    # Unknown 사용자는 DB에 저장하지 않음
    if not worker or worker.lower() in ['unknown', '알수없음', '알 수 없음', '미확인']:
        logging.debug(f"[MongoDB] Unknown 사용자 저장 건너뜀: worker={worker}")
        return
    
    try:
        from database import get_database
        from state import (violation_batch_queue, violation_batch_lock, 
        ...
        # ⭐ 전역 쿨다운 체크: 5초에 1건만 저장
        with violation_batch_lock:
            if current_time - state.global_last_saved_time < GLOBAL_SAVE_INTERVAL:
                remaining = GLOBAL_SAVE_INTERVAL - (current_time - state.global_last_saved_time)
                logging.debug(f"[MongoDB] 전역 쿨다운 중: {remaining:.1f}초 남음 (worker={worker})")
                return
        ...
        # 전역 타임스탬프 업데이트
        with violation_batch_lock:
            state.global_last_saved_time = current_time
        ...
        logging.info(f"[MongoDB] 위반 저장: {worker} - {violation_type} (CAM-{cam_id})")
...
def update_model_results_from_frame(result_data: Dict[str, Any], cam_id: int = 0) -> None:
    ...
    with results_lock:
        ...
        for violation in violations_list:
            ...
            # 예: ["안전모", "마스크"]와 ["마스크", "안전모"]는 같은 키로 인식
        ...
        for v in violations_list:
            violations = v.get("violations", [])
            if any("안전모" in str(vt) or "마스크" in str(vt) or "안전조끼" in str(vt) for vt in violations):
                safety_violation_count += 1
    ...
    try:
        while True:
            ...
            with queue_lock:
                ...
                temp_frames = []
                current_time = time.time()
                # 스트림 지연 방지: 오래된 프레임 스킵 기준 완화 (프레임 손실 방지)
                MAX_FRAME_AGE = 0.2  # 0.1초 -> 0.2초 (프레임 손실 방지)
                # 처리 속도 향상: 확인할 프레임 수 증가 (더 많은 프레임 확인)
                MAX_QUEUE_CHECK = 15  # 10 -> 15 (더 많은 프레임 확인)
                ...
                if temp_frames:
                    ...
                    # 나머지 프레임은 다시 큐에 넣기 (프레임 손실 방지)
                    # 단, 큐가 가득 차면 버림
                    for remaining_frame in temp_frames[1:]:
                        try:
                            frame_queues[cam_id].put_nowait(remaining_frame)
                        except queue.Full:
                            # 큐가 가득 차면 버림
                            pass
                else:
                ...
                if cam_id in frame_queues and frame_queues[cam_id] is not None:
                    ...
                    # 프레임 큐 상태 확인 (병목 분석용)
                    if remaining_queue > 5:  # 큐가 5개 이상 쌓이면 경고
                        logging.warning(f"[QUEUE CAM-{cam_id}] ⚠️ 프레임 큐 쌓임: {remaining_queue}개 대기 중 (병목 가능성)")
            ...
            try:
                ...
                # 스트림 지연 방지: 타임아웃 단축
                # FPS 기반 동적 타임아웃 조정 (프레임 드랍 방지)
                dynamic_timeout = 0.4  # 기본값: 0.5 -> 0.4초
                try:
                    with frame_stats_lock:
                        ...
                        if len(recent_frames) >= 2:
                            ...
                            if time_span > 0:
                                ...
                                # FPS 기반 타임아웃 조정 (더 짧게 설정)
                                if current_fps >= 30:
                                    dynamic_timeout = 0.2  # 0.3 -> 0.2초 (높은 FPS: 짧은 타임아웃)
                                elif current_fps >= 20:
                                    dynamic_timeout = 0.3  # 0.4 -> 0.3초 (중간 FPS: 중간 타임아웃)
                                elif current_fps >= 15:
                                    dynamic_timeout = 0.4  # 0.5 -> 0.4초 (낮은 FPS: 기본 타임아웃)
                                else:
                                    dynamic_timeout = 0.5  # 0.7 -> 0.5초 (매우 낮은 FPS: 긴 타임아웃)
                except Exception:
                    pass  # 예외 시 기본값 사용
                ...
                processed_frame_bytes, result_data = await asyncio.wait_for(
                    ...
                    timeout=dynamic_timeout  # FPS 기반 동적 타임아웃
                )
                ...
                # 프레임 통계 업데이트 (FPS 계산용) - latest_frames 업데이트 시점에만 카운트
                # 이렇게 하면 WebSocket과 MJPEG 스트림 모두에서 중복 카운트 방지
                with frame_stats_lock:
                ...
                if result_data:
                    ...
                    try:
                        ...
                        # ConnectionResetError 처리 강화 (Windows WinError 10054)
                        error_msg = str(conn_err)
                        if "10054" in error_msg or "강제로 끊겼습니다" in error_msg:
                            logging.debug(f"[CAM-{cam_id}] 클라이언트 연결 종료 (클라이언트 {client_id}): {conn_err}")
                        else:
...
async def websocket_handler(request: web.Request):
    ...
    try:
        ...
        async def broadcast_worker_results():
            ...
            - camera_worker의 camera_buffers[cam_id]["latest_result"]를 읽어서 그대로 전달
            - 타임스탬프 비교 대신, 전송 간격(최소 80ms)을 기준으로 전송하여
              타임스탬프 불일치로 인한 전송 누락을 방지
            """
            from camera_worker import get_camera_buffer
            from state import get_latest_cache
            last_sent_time = 0.0  # 마지막 전송 시각 (time.time 기준)
            MIN_SEND_INTERVAL = 0.08  # 최소 전송 간격 (초) - 약 12.5 FPS
            
            while True:
                try:
                    await asyncio.sleep(1 / 60)  # 60fps로 체크
                    ...
                    # 1순위: 통합 결과 캐시(state.model_results_cache)에서 직접 가져오기
                    # 백엔드 로그에 기록되는 위반/얼굴 결과와 동일한 소스를 사용
                    result = None
                    try:
                        # 캐시 유효 시간을 3.0초로 설정 (감지 결과 안정성 확보)
                        from state import CACHE_TTL
                        cached = get_latest_cache(cam_id, max_age=CACHE_TTL)
                    except Exception:
                        cached = None
                    ...
                    if cached:
                        ...
                        # 2순위: camera_worker가 넣어둔 latest_result 사용
                    ...
                    if "ts_ms" not in result:
                        result["ts_ms"] = int(result["timestamp"] * 1000)
                    ...
                    try:
                        ...
                        result_json = json.dumps(message, ensure_ascii=False)
                        await ws.send_str(result_json)
                        last_sent_time = now

                        # 전송 로그 (항상 출력 - 디버깅용)
                        faces_count = len(result.get('recognized_faces', []))
                        violations_count = len(result.get('violations', []))
                        logging.info(
                            f"[CAM-{cam_id}] ✅ WebSocket 전송: bytes={len(result_json)}, "
                            f"faces={faces_count}, violations={violations_count}"
                        )
                        # 빈 데이터인 경우 경고
                        if faces_count == 0 and violations_count == 0:
                            logging.warning(
                                f"[CAM-{cam_id}] ⚠️ 빈 결과 전송됨! "
        ...
        async for msg in ws:
            ...
            if msg.type == WSMsgType.BINARY:
                ...
                with queue_lock:
                    ...
                    # 큐가 가득 찬 경우 오래된 프레임만 제거 (프레임 손실 최소화)
                    # 큐가 가득 차면 오래된 프레임만 제거하여 최신 프레임 보존
                    if frame_queue.full():
                        dropped_count = 0
                        # 오래된 프레임만 제거 (큐 크기의 30%만 제거)
                        drain_count = max(1, int(frame_queue.maxsize * 0.3))  # 0.7 -> 0.3 (30%만 제거)
                        temp_old_frames = []
                        for _ in range(drain_count):
                            try:
                                old_frame = frame_queue.get_nowait()
                                frame_ts = old_frame[1] if isinstance(old_frame, tuple) and len(old_frame) > 1 else time.time()
                                ...
                                if frame_age > 0.3:
                                    ...
                                    # 최신 프레임은 다시 넣기
                                    temp_old_frames.append(old_frame)
                            except queue.Empty:
                                break
                        # 최신 프레임은 다시 넣기
                        for old_frame in temp_old_frames:
                            try:
                                frame_queue.put_nowait(old_frame)
                            except queue.Full:
                                pass
                    
                    # 최신 프레임 추가 (큐가 가득 차면 오래된 프레임 제거 후 추가)
                    try:
                        frame_queue.put_nowait((frame_bytes, time.time()))
                    except queue.Full:
                        # 큐가 가득 차면 가장 오래된 프레임 하나만 제거하고 추가
                        try:
                            frame_queue.get_nowait()  # 가장 오래된 프레임 제거
                            frame_queue.put_nowait((frame_bytes, time.time()))  # 최신 프레임 추가
                        except queue.Empty:
                            pass
...
async def api_violation_image_handler(request: web.Request):
    ...
    try:
        ...
        if os.path.isabs(image_path):
            possible_paths.append(os.path.normpath(image_path))
        else:
            # 2. /images/로 시작하는 상대 경로인 경우
            if image_path.startswith('/images/') or image_path.startswith('images/'):
                # 파일명만 추출하여 logs 폴더와 결합
                filename = os.path.basename(image_path)
                possible_paths.append(os.path.join(log_folder, filename))
            else:
                # 3. 상대 경로인 경우 (파일명만 있는 경우 포함)
                filename = os.path.basename(image_path)
                # logs 폴더 기준으로 시도
                possible_paths.append(os.path.join(log_folder, filename))
                # 원본 경로 그대로도 시도 (상대 경로인 경우)
        ...
        for path_to_try in possible_paths:
            ...
            if os.path.exists(path_to_try):
                ...
                logging.debug(f"[이미지 API] 파일 찾음: {found_path} (원본: {original_path})")
                break
        
        # 파일을 찾지 못한 경우 - 타임스탬프 기반 검색 시도
        if not found_path:
            # 타임스탬프 패턴 추출 시도 (예: violation_20251203_100513_0.jpg -> 20251203_100513)
            import re
            filename = os.path.basename(image_path)
            
            # 패턴 1: violation_YYYYMMDD_HHMMSS_*.jpg
            match1 = re.search(r'(\d{8}_\d{6})', filename)
            if match1:
                timestamp_pattern = match1.group(1)
                logging.debug(f"[이미지 API] 타임스탬프 패턴 추출: {timestamp_pattern}")
                
                # logs 폴더에서 해당 타임스탬프로 시작하는 파일 검색
                try:
                    if os.path.exists(log_folder):
                        files = os.listdir(log_folder)
                        jpg_files = [f for f in files if f.lower().endswith('.jpg')]
                        # 타임스탬프로 시작하는 파일 찾기
                        matching_files = [f for f in jpg_files if f.startswith(timestamp_pattern)]
                        
                        if matching_files:
                            # 첫 번째 매칭 파일 사용
                            found_path = os.path.join(log_folder, matching_files[0])
                            logging.info(f"[이미지 API] 타임스탬프 기반 매칭: {found_path} (원본: {original_path})")
                except Exception as search_err:
                    logging.warning(f"[이미지 API] 타임스탬프 기반 검색 실패: {search_err}")
            
            # 여전히 찾지 못한 경우
            if not found_path:
                logging.warning(f"[이미지 API] 파일이 존재하지 않음: {original_path}")
                logging.warning(f"[이미지 API] 시도한 경로들: {possible_paths}")
...
async def api_frontend_logs_handler(request: web.Request):
    ...
    try:
        ...
        for log_entry in logs:
            ...
            # 에러 객체를 문자열로 변환 (프론트엔드 작업자 조회 오류 개선)
...
async def api_workers_post_handler(request: web.Request):
    ...
    try:
        ...
        try:
            ...
            # 작업자 목록 캐시 무효화 (새 작업자가 즉시 보이도록)
            workers_cache.clear()
            logging.info(f"[API] 작업자 생성 완료 - 캐시 무효화: workerId={worker_id}")
...
async def api_workers_put_handler(request: web.Request):
    ...
    try:
        ...
        try:
            ...
            # 작업자 목록 캐시 무효화 (업데이트된 작업자가 즉시 보이도록)
            workers_cache.clear()
            logging.info(f"[API] 작업자 업데이트 완료 - 캐시 무효화: workerId={worker_id}")
...
async def api_workers_delete_handler(request: web.Request):
    ...
    try:
        ...
        try:
            ...
            # 작업자 목록 캐시 무효화 (삭제된 작업자가 즉시 사라지도록)
            workers_cache.clear()
            logging.info(f"[API] 작업자 삭제 완료 - 캐시 무효화: workerId={worker_id}")
                                ...
                                if face_analyzer and face_database:
                                    try:
                                        person_name, similarity_score, embedding, face_bbox = _process_face_recognition(
                                            ...
                                            face_model=None,  # buffalo_l 사용 시 None
...
async def api_detect_face_position_handler(request: web.Request):
    ...
    try:
        ...
        try:
            ...
            # buffalo_l 사용 시 face_analyzer로 체크
            face_analyzer = getattr(safety_system_instance, 'face_analyzer', None) if safety_system_instance else None
            if safety_system_instance is None or face_analyzer is None:
            ...
            for face in faces:
                ...
                faces_with_score.append((bbox, score))
            
            # confidence 기준으로 정렬
            faces_with_score.sort(key=lambda x: x[1], reverse=True)
            
            if not faces_with_score:
                return web.json_response({
                    'success': True,
                    'has_face': False,
                    'face_detected': False
            ...
            # 첫 번째 얼굴의 위치 사용
            biggest_bbox, biggest_conf = faces_with_score[0]
...
<p>API 문서: <a href="/api/health">/api/health</a></p>
    ...
    async def mjpeg_stream(request: web.Request):
        ...
        target_fps = 60  # 60 FPS (최대 부드러움)
        frame_interval = 1.0 / target_fps  # 약 0.0167초
        ...
        try:
            while True:
                                ...
                                if get_camera_buffer is not None:
                                    try:
                                        ...
                                        if buffer and buffer.get("latest_frame") is not None:
                                            ...
                                            ret, buffer_jpeg = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 70])
                                            if ret:
                ...
                if frame_bytes:
                    ...
                    # MJPEG 스트림에서도 frame_stats 업데이트 (프론트엔드가 MJPEG 스트림 사용)
                    # 중복 카운트 방지: 최근 업데이트 시간 체크
                    current_time = time.time()
                    with frame_stats_lock:
                        if cam_id not in frame_stats:
                            frame_stats[cam_id] = {
                        ...
                        stats = frame_stats[cam_id]
                        # 중복 카운트 방지: 최근 0.01초(10ms) 이내 업데이트가 없을 때만 카운트
                        # (WebSocket과 MJPEG 스트림이 동시에 카운트하는 것을 방지)
                        time_since_last = current_time - stats.get('last_frame_time', 0)
                        if time_since_last >= 0.01:  # 10ms 이상 경과했을 때만 카운트
                            stats['frame_count'] += 1
                            stats['last_frame_time'] = current_time
                            stats['frame_times'].append(current_time)
                            if len(stats['frame_times']) > 60:
...
def main():
    ...
    async def initialize_safety_system():
        ...
        logging.info(f"  - Face 모델: buffalo_l (InsightFace)")
        logging.info(f"  - Fall 모델: {config.Paths.YOLO_FALL_DETECTION_ENGINE}")
        
        # 모델 파일 존재 여부 확인
        import os
        violation_exists = os.path.exists(config.Paths.YOLO_VIOLATION_MODEL)
        pose_exists = os.path.exists(config.Paths.YOLO_POSE_MODEL)
        fall_exists = os.path.exists(config.Paths.YOLO_FALL_DETECTION_ENGINE)
        ...
        logging.info(f"  - Fall 모델: {'✅ 존재' if fall_exists else '❌ 없음'}")
        ...
        while retry_count < max_retries:
            try:
                ...
                if temp_check and temp_check.violation_model and temp_check.pose_model:
                    ...
                    if temp_check.face_analyzer:
                        logging.info("✅ 얼굴 인식 모델도 로드됨 (buffalo_l)")
                        logging.info(f"   - Face Analyzer (buffalo_l): {temp_check.face_analyzer is not None}")
                    logging.info("=" * 60)
                    ...
                    try:
                        ...
                        if temp_check.violation_model:
                            ...
                            # ONNX 모델은 device 파라미터를 전달하지 않음 (ONNX Runtime이 자동 처리)
                            temp_check.violation_model(dummy_frame, verbose=False, half=True)
                            
                        if temp_check.pose_model:
                            logging.info("  - YOLO Pose Warmup...")
                            # ONNX 모델은 device 파라미터를 전달하지 않음 (ONNX Runtime이 자동 처리)
                            temp_check.pose_model(dummy_frame, verbose=False, half=True)
                            
                        # 얼굴 인식 Warmup (buffalo_l)
                        if temp_check.face_analyzer:
                            logging.info("  - Face Detection Warmup (buffalo_l)...")
                            _ = temp_check.face_analyzer.get(dummy_frame)
    ...
    async def start_server():
        ...
        async def start_camera_workers():
            ...
            try:
                ...
                # 적응형 워커 관리자 제거됨 (불필요한 복잡성 제거)
                logging.info("✅ 워커 설정 완료 (고정 워커 수 사용)")
        ...
        async def monitor_initialization():
            """SafetySystem 초기화 상태 모니터링 (타임아웃 설정)"""
            check_count = 0
            max_wait_time = 60  # 최대 대기 시간: 60초
            max_checks = max_wait_time // 5  # 5초마다 확인하므로 12회
            ...
            while True:
                ...
                if check_count > max_checks:
                    logging.error(f"❌ SafetySystem 초기화 타임아웃 ({max_wait_time}초 초과). 모델 파일을 확인하세요.")
                    break  # 타임아웃 시 모니터링 종료
        ...
        # GPU 사용량 주기적 모니터링 (10초마다 - 더 자주 체크)
        async def monitor_gpu_usage():
            """주기적으로 GPU 사용량 체크 및 최적화 권장사항 출력"""
            await asyncio.sleep(10)  # 서버 시작 후 10초 대기
            while True:
                try:
                    from utils_server import get_gpu_usage_stats
                    from state import face_recognition_executor, yolo_executor, dangerous_behavior_executor, frame_processing_executor, frame_queues, queue_lock
                    
                    # GPU 사용률 확인
                    gpu_stats = get_gpu_usage_stats()
                    
                    # 워커 상태 확인
                    face_max = face_recognition_executor._max_workers
                    yolo_max = yolo_executor._max_workers
                    danger_max = dangerous_behavior_executor._max_workers
                    frame_max = frame_processing_executor._max_workers
                    
                    # 프레임 큐 크기 확인
                    total_queue_size = 0
                    with queue_lock:
                        for cam_id, q in frame_queues.items():
                            total_queue_size += q.qsize()
                    
                    # 상세 GPU/워커 상태 로깅
                    logging.info("=" * 80)
                    logging.info("📊 GPU/워커 상태 모니터링 (10초 주기)")
                    logging.info("=" * 80)
                    
                    if gpu_stats:
                        for gpu_id, stat in gpu_stats.items():
                            logging.info(f"GPU {gpu_id} ({stat['name']}):")
                            logging.info(f"  메모리: {stat['memory_reserved_gb']:.2f}GB / {stat['memory_total_gb']:.2f}GB ({stat['memory_util_percent']:.1f}%)")
                            logging.info(f"  할당됨: {stat['memory_allocated_gb']:.2f}GB, 여유: {stat['memory_total_gb'] - stat['memory_reserved_gb']:.2f}GB")
                    else:
                        logging.info("GPU 통계 수집 실패")
                    
                    logging.info(f"워커 수: Face={face_max}, YOLO={yolo_max}, Danger={danger_max}, Frame={frame_max}")
                    logging.info(f"프레임 큐 총 크기: {total_queue_size}")
                ...
                await asyncio.sleep(10)  # 10초마다 체크 (5분 -> 10초로 변경)
        ...
        # 서버 종료: 모든 태스크 취소 (Event loop closed 에러 방지)
        logging.info("서버 종료 중...")
        try:
            # 모든 실행 중인 태스크 취소
            tasks = [task for task in asyncio.all_tasks() if task is not asyncio.current_task()]
            if tasks:
                logging.info(f"실행 중인 태스크 {len(tasks)}개 취소 중...")
                for task in tasks:
                    task.cancel()
                # 태스크 취소 대기 (최대 5초)
                await asyncio.gather(*tasks, return_exceptions=True)
                logging.info("모든 태스크 취소 완료")
        except Exception as e:
            logging.warning(f"태스크 취소 중 오류 (무시): {e}")
