...
# PPE 합성된 이미지가 있으면 data/images 사용 (원본 + PPE 합성 이미지 모두 포함)
# 없으면 원본 이미지 폴더 사용
...
def build_database():
    ...
    # 1. YOLOv8n-Face 모델 로드 (TensorRT 엔진 우선, 키포인트 제공!)
    ...
    if os.path.exists(yolo_face_engine_path):
        print(f"YOLOv8n-Face TensorRT 엔진 로딩 중: {yolo_face_engine_path}")
        yolo_face_model = YOLO(yolo_face_engine_path, task='pose')
        print(f"✅ YOLOv8n-Face TensorRT 로드 완료 (GPU 가속!)")
    elif os.path.exists(yolo_face_pt_path):
        print(f"YOLOv8n-Face PT 모델 로딩 중: {yolo_face_pt_path}")
        yolo_face_model = YOLO(yolo_face_pt_path)
        print(f"✅ YOLOv8n-Face PT 로드 완료")
    else:
    ...
    for root, dirs, files in os.walk(DB_PATH):
        ...
        for file in image_files:
            ...
            for aug_idx, augmented_img in enumerate(images_to_process):
                ...
                try:
                    yolo_results = yolo_face_model(augmented_img, conf=0.3, verbose=False)
