        ...
        try:
            ...
            if self.use_tensorrt and self.trt_context is not None:
                ...
                if embedding is not None:
                    embedding_method = "TensorRT"
                    logging.info(f"[FAST_EMB] TensorRT 임베딩 추출: {embedding_time:.1f}ms, shape={embedding.shape}")
            ...
            if embedding is None and self.use_direct_onnx and self.session is not None:
                ...
                embedding_time = (time.time() - embedding_start) * 1000
                if embedding is not None:
                    embedding_method = "ONNX"
                    logging.info(f"[FAST_EMB] ONNX 임베딩 추출: {embedding_time:.1f}ms, shape={embedding.shape}")
            ...
            if embedding is None and face_analyzer is not None:
                ...
                embedding_time = (time.time() - embedding_start) * 1000
                if embedding is not None:
                    embedding_method = "InsightFace"
                    logging.info(f"[FAST_EMB] InsightFace 임베딩 추출: {embedding_time:.1f}ms, shape={embedding.shape}")
